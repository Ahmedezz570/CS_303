...
const Mobiles = () => {
  ...
  useEffect(() => {
    const filteredData = daata.filter((item) => item.name === "sweatshirt");
  ...
  const handleSearch = (text: string) => {
    ...
    const filteredData = daata.filter(
      (item) => item.name === "sweatshirt" && item.name.toLowerCase().includes(text.toLowerCase())
    );
    setData(filteredData);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchBox}>
        <Text style={styles.title}>sweatshirt</Text>
        <TextInput
