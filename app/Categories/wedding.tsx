...
const wedding = () => {
  ...
  useEffect(() => {
    const filteredData = daata.filter((item) => item.name === "wedding suit");
  ...
  const handleSearch = (text: string) => {
    ...
    const filteredData = daata.filter(
      (item) => item.name === "wedding suit " && item.name.toLowerCase().includes(text.toLowerCase())
    );
    setData(filteredData);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchBox}>
        <Text style={styles.title}>wedding suit</Text>
        <TextInput
